...
#building_types = ['R4-ELEVATOR','R2-WALK-UP','R9-CONDOPS','C1-WALK-UP','D1-ELEVATOR','D7-ELEVATOR','C7-WALK-UP','D9-ELEVATOR','D5-ELEVATOR','D6-ELEVATOR','D8-ELEVATOR','D3-ELEVATOR','C4-WALK-UP','D2-ELEVATOR','C5-WALK-UP',
...
def check_website(word):
    ...
    #lis = list(word)
    #test_df = spark.createDataFrame(lis, StringType())
    #test_df.show()
    #test_df.createOrReplaceTempView("test_df")
    #test_df = spark.sql("select value as Name from test_df")
    #pred = model.transform(word)
    #pred.show(pred.count())
    #sc3 = SparkContext()
    #sameModel = LogisticRegressionModel.load('/user/sna354/class')
   ...
   #CHECKING FOR ZIP CODES
    ...
    #VEHICLE TYPE
    #for subs in vehicles:
    #    if subs.lower() in word.strip().lower():
    #        return "Vehicle Type"
    #    if word.strip().lower() in subs.lower():
    #        return "Vehicle Type"
    ...
    #for list_name in color:
    #    ls.append((fuzz.partial_ratio(word.lower(), list_name.lower()), list_name))
    #ls.sort(reverse=True)
    #return str(ls)
    #if ls[0][0] > 90:
    #    return ls[0][0]
...
web1 = sqlContext.read.option("sep", "\t").option("header", "false").option("delimiter","\t").schema(schema).csv(sys.argv[1])
print(json_for_website(web1, "Name"))
